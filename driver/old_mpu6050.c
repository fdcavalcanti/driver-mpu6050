 ...
 *
 */
static void MPU_Burst_Read(unsigned char start_reg, unsigned int length,
 ...
 */
static ssize_t mpu_read(struct file *File, char *user_buffer, size_t count,
                        loff_t *offs) {
 ...
 */
static ssize_t mpu_write(struct file *File, const char *user_buffer,
...
*/
static const struct i2c_device_id mpu_id[] = {{SLAVE_DEVICE_NAME, 0}, {}};
...
static int __init ModuleInitialization(void) {
  ...
  if (mpu_i2c_adapter != NULL) {
    mpu_i2c_client =
      i2c_new_client_device(mpu_i2c_adapter, &mpu_i2c_board_info);
    if (mpu_i2c_client != NULL) {
      i2c_add_driver(&mpu_driver);
      ret = 0;
    }
    // Get adapter will increment adapter number by 1.
    // This call will go back by 1.
